...
try:
    for variant, commit in variants.items():
        ...
        (reports / f'build-{variant}.json').write_text(json.dumps({
            ...
            'normalized_config_sha256': hashlib.sha256(config.encode()).hexdigest(),
...
for round_number, order in enumerate([list(variants), list(reversed(variants))], 1):
    for variant in order:
        report = reports / variant / f'round-{round_number}'
        report.mkdir(parents=True, exist_ok=True)
        files = output / 'parquet' / variant / f'round-{round_number}'
        files.mkdir(parents=True, exist_ok=True)
        common = ['--iterations', '7', '--partitions', '1', '--batch-size', '2048', '--memory-limit', '2G', '--mem-pool-type', 'greedy']
        ...
        for name, args in cases:
            run([str(binaries / variant), *args, *common, '--output', str(report / f'{name}.json')], report / f'{name}.log', cwd=engine / 'benchmarks')
        layouts = []
        for file in sorted(files.glob('*.parquet')):
            parquet = pq.ParquetFile(file)
            meta = parquet.metadata
            assert meta.num_rows == 131072 and parquet.read().num_rows == 131072
            groups = []
            ...
            layouts.append({'case': file.stem, 'rows': meta.num_rows, 'file_bytes': file.stat().st_size, 'groups': groups})
