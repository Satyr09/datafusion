...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn changing_widths_round_trip() {
    ...
    assert!(actual.num_row_groups() >= expected.num_row_groups());
}
...
async fn nested_columns_use_root_row_counts() {
    ...
    assert!(actual.num_row_groups() > 0);
}
...
#[test]
fn projection_preserves_fractional_bytes_and_handles_large_values() {
    let progress = ColumnProgress {
        rows: 1024,
        bytes: 128,
    };
    assert_eq!(progress.projected_bytes(1024), 128);
    assert_eq!(progress.projected_bytes(2048), 256);
    assert_eq!(progress.projected_bytes(1025), 129);
    assert_eq!(
        (0..100)
            .map(|_| progress.projected_bytes(1024))
            .sum::<usize>(),
        12_800
    );
    let huge = ColumnProgress {
        rows: 1,
        bytes: usize::MAX,
    };
    assert_eq!(huge.projected_bytes(usize::MAX), usize::MAX);
    // Unequal progress: each numerator must use its own root-row count.
    let fast = ColumnProgress {
        rows: 200,
        bytes: 100,
    };
    let slow = ColumnProgress {
        rows: 50,
        bytes: 1000,
    };
    assert_eq!(fast.projected_bytes(200) + slow.projected_bytes(200), 4100);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn feedback_lag_is_bounded_by_channel_capacity() {
    for capacity in [1, 2, 8] {
        let batch = strings(128, 1024);
        let props = properties(100_000, Some(10_000_000));
        let writer =
        ...
        let mut group = InProgressRowGroup::new(&factory, 0, &ctx, &Time::new()).unwrap();
        let mut cumulative_rows = vec![0];
        for write in 0..40 {
            let rows = 17 + write % 31;
            group
                .write(&batch.slice(0, rows), &ctx.schema)
                .await
                .unwrap();
            group.after_write().await.unwrap();
            cumulative_rows.push(group.rows);
            for receiver in &group.progress {
                let progress = *receiver.borrow();
                let acknowledged = cumulative_rows.binary_search(&progress.rows).unwrap();
                assert!(cumulative_rows.len() - 1 - acknowledged <= capacity + 1);
                assert!(progress.projected_bytes(group.rows) >= progress.bytes);
            }
        }
        group.synchronize().await.unwrap();
        assert!(
            group
                .progress
                .iter()
                .all(|rx| rx.borrow().rows == group.rows)
        );
    }
}
