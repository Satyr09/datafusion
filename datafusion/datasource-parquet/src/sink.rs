...
impl ColumnProgress {
    fn projected_bytes(self, dispatched_rows: usize) -> usize {
        if self.rows == 0 {
            return 0;
        }
        debug_assert!(self.rows <= dispatched_rows);
        // Multiply before dividing so sub-byte per-row contributions from
        // many columns do not disappear. At an acknowledged prefix this is
        // exactly the reported size; otherwise it is a prediction, not a bound.
        let projected =
            (self.bytes as u128 * dispatched_rows as u128).div_ceil(self.rows as u128);
...
impl InProgressRowGroup {
    ...
    /// Project coherent column observations onto dispatched root rows. Each
    /// channel holds at most C queued slices plus one being encoded: at a full
    /// dispatch boundary a healthy worker can lag by at most C + 1 slices.
    /// This bounds feedback lag, not byte overshoot or final compressed size.
    fn estimated_bytes(&self) -> usize {
        self.progress.iter().fold(0usize, |total, rx| {
            let progress = *rx.borrow();
            total.saturating_add(progress.projected_bytes(self.rows))
        })
    }
...
/// Byte estimates are projected from bounded, asynchronous feedback.
