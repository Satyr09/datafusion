...
/// running row / write counts. Reset together at each row-group boundary.
...
/// Estimates the in-progress row group's encoded size by summing each column
/// writer's latest reported `get_estimated_total_bytes()`, after waiting for
/// every column to have applied `writes_dispatched` writes. The wait makes the
/// estimate exactly as fresh as the single-threaded `ArrowWriter`'s (batch
/// granularity) and keeps row-group boundaries deterministic.
///
/// Returns `None` if a column task has exited (the plan is shutting down).
async fn current_rg_bytes_estimate(
    ...
    for rx in col_progress_rxs.iter_mut() {
        let progress = rx
            .wait_for(|p| p.writes_done >= writes_dispatched)
            .await
            .ok()?;
        total += progress.estimated_bytes;
    }
    Some(total)
}
        ...
        while let Some(mut rb) = data.recv().await {
            ...
            loop {
                ...
                let remaining_by_bytes = if !track_progress || rg.rows == 0 {
                    ...
                    let Some(est) = current_rg_bytes_estimate(
                        &mut rg.col_progress_rxs,
                        rg.writes_dispatched,
                    )
                    .await
                    else {
                        return Ok(());
                    ...
                    if est >= max_bytes {
                        ...
                        let avg_row_bytes = (est / rg.rows).max(1);
                        (max_bytes - est) / avg_row_bytes
                    }
                };
