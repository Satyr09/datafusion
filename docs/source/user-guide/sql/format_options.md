...
| MAX_ROW_GROUP_BYTES                        | No                      | Target maximum estimated encoded row-group size in bytes. Either this target or `MAX_ROW_GROUP_SIZE` triggers a flush. The first batch is accepted subject to the row limit; prediction can overshoot the byte target. Parallel byte boundaries are approximate and may vary between runs.                                           | `'max_row_group_bytes'`                               | None                     |
